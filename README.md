...
### Three answering modes, chosen per question

The **Gemini | Ollama | Local** control beside *Send* puts the same question to a
different stack, so the difference is observable rather than asserted:

| Mode | Retrieval | Answer | Needs |
|---|---|---|---|
| Gemini | Gemini embeddings | Generated by Gemini | `GEMINI_API_KEY` |
| Ollama | The corpus's existing index | Generated by an Ollama Cloud model | `OLLAMA_API_KEY` |
| Local | Lexical | Quoted from the policy | Nothing |

**Ollama runs on Ollama Cloud — nothing is downloaded or stored locally.** Create
a key at <https://ollama.com/settings/keys> and set `OLLAMA_API_KEY` in `.env`.
Ollama Cloud offers no embeddings API, so Ollama mode reuses the index already
built (by default the Gemini one) and only the answer comes from Ollama. That
gives a second, independently metered source of generated answers when the
